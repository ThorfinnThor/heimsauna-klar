...
The L-16 maintenance process is deterministic, read-only and compatible with the static export. It does not alter product JSON, publication switches or deployable output. The current report covers all 18 US product candidates and keeps them outside public catalog resolution.
...
- `npm run us:catalog:review -- --as-of 2026-09-14`: 18 products, 18 awaiting first review, 0 stale sources, 0 missing source references, 0 due reviews, 0 invalid schedules
...
L-16 is accepted as a prepared operating process. The 18 products still require the documented first technical review before promotion. S-24 remains dependent on explicit operator authorization, and US affiliate output and image use remain unavailable until their separate account and rights evidence exists.
