...
Technical inventory date: 2026-09-15
...
- The browser contains no first-party analytics SDK, advertising pixel, cookie write, local storage, session storage, browser `fetch`, `XMLHttpRequest`, `sendBeacon` or third-party iframe. Cloudflare Web Analytics is active as a hosting-layer service and may inject its own performance beacon at the edge.
...
1. Cloudflare Web Analytics is enabled for the production domain. Its provider-side retention and account settings remain outside the repository.
