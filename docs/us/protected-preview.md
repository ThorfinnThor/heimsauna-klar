...
The runner validates the source bundle, sets the research-preview environment only for its child processes, generates the static export and checks the 19 US pages before deletion. It verifies that all six real candidate products reach the catalog, Finder payload and their exact product pages with configuration and source data. It also checks the contact, methodology, affiliate-disclosure and privacy drafts through the same static preview path. All pages must remain `noindex, follow`, must carry a research-preview notice and must not contain synthetic test markers, affiliate labels or public Product schema.
