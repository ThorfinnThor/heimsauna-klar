...
const records = [
  ...
  { id: "finnleo-northstar-indoor-sauna-4-x-6-9630-2263", capacity: 2, dimensions: dimensions(72, 48, 84), voltage: 240, amps: 30, requiredCircuit: null, breaker: null, connection: null, dedicated: null, power: 4500, material: "Nordic White Spruce interior and exterior; Clear Abachi or Aspen benches and backrests", url: "https://www.finnleo.com/products/northstar-indoor-sauna-4-x-6-9630-2263", form: "Indoor traditional sauna cabin" },
  { id: "finnleo-northstar-indoor-sauna-5-x-6-9630-2264", capacity: 2, capacityConflict: { description: 3, field: 2 }, dimensions: dimensions(72, 60, 84), voltage: 240, amps: 30, requiredCircuit: null, breaker: null, connection: null, dedicated: null, power: 6000, material: "Nordic White Spruce interior and exterior; Clear Aspen or Abachi benches and backrests", url: "https://www.finnleo.com/products/northstar-indoor-sauna-5-x-6-9630-2264", form: "Indoor traditional sauna cabin" },
  { id: "finnleo-northstar-indoor-sauna-5-x-7-9630-2265", capacity: 5, capacityRange: [4, 5], dimensions: dimensions(84, 60, 84), voltage: 240, amps: 30, requiredCircuit: null, breaker: null, connection: null, dedicated: null, power: 6000, material: "Nordic White Spruce interior and exterior; Clear Abachi or Aspen benches and backrests", url: "https://www.finnleo.com/products/northstar-indoor-sauna-5-x-7-9630-2265", form: "Indoor traditional sauna cabin" },
...
for (const record of records) {
  ...
  const capacityDescriptionEvidenceId = `evidence-${record.id}-capacity-description-2026-09-22`;
  const capacityFieldEvidenceId = `evidence-${record.id}-capacity-field-2026-09-22`;
  ...
  sources.evidence.push(
    { id: productEvidenceId, entity_id: record.id, source_id: sourceId, field_path: "product_identity_capacity_heat_and_placement", raw_value: `${product.model ?? record.id} is listed on the Finnleo model page${record.capacityConflict ? `; its description says it seats ${record.capacityConflict.description}, while the seating-capacity field says ${record.capacityConflict.field}` : record.capacityRange ? ` with a seating-capacity range of ${record.capacityRange[0]}-${record.capacityRange[1]}` : ` with a seating capacity of ${record.capacity}`}; the page identifies the model as an indoor ${record.heat ?? "traditional"} sauna.` },
    ...
    { id: electricalEvidenceId, entity_id: configuration.id, source_id: sourceId, field_path: "electrical_supply_options", raw_value: record.breaker ? `Finnleo says to hardwire this model to a ${record.voltage} V ${record.breaker}-amp breaker and lists ${record.power} W and ${record.amps} A; it does not state that the circuit is dedicated.` : `Finnleo lists ${record.voltage ?? "no voltage"} V, ${record.amps ?? "no amperage"} A, ${record.connection ?? "no normalized connection wording"} and ${record.power ?? "no single rated power"} W for this model; it does not label the amperage as a required circuit or breaker rating.` },
  );
  if (record.capacityConflict) {
    sources.evidence.push(
      { id: capacityDescriptionEvidenceId, entity_id: configuration.id, source_id: sourceId, field_path: "manufacturer-product-page.description.capacity", raw_value: `The official description says the cabin can comfortably seat ${record.capacityConflict.description}.` },
      { id: capacityFieldEvidenceId, entity_id: configuration.id, source_id: sourceId, field_path: "manufacturer-product-page.seating-capacity", raw_value: `The official seating-capacity field states ${record.capacityConflict.field}.` },
    );
  }
  ...
  configuration.manufacturer_sku = unknown("The reviewed Finnleo page does not explicitly label a manufacturer SKU; the URL slug is not treated as one.");
  configuration.capacity.seated = record.capacityConflict
    ? { status: "conflict", evidence_ids: [capacityDescriptionEvidenceId, capacityFieldEvidenceId], note: `The official description says ${record.capacityConflict.description}, while the seating-capacity field says ${record.capacityConflict.field}.` }
    : documented(record.capacity, productEvidenceId);
  ...
  requirement.required_circuit_a = record.requiredCircuit ? documented(record.requiredCircuit, electricalEvidenceId) : unknown("The exact Finnleo model page does not state a required circuit rating.");
  requirement.specified_breaker_a = record.breaker ? documented(record.breaker, electricalEvidenceId) : unknown("The exact Finnleo model page does not state a breaker rating.");
