...
test("real pilot data produces fourteen known indoor infrared 120 V results", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "meets-known-criteria").length, 17);
  ...
  assert.deepEqual(results.filter((entry) => entry.status === "meets-known-criteria").map((entry) => entry.productId), [
    "peak-shasta", "peak-everest", "jnh-tosi-1", "jnh-tosi-2", "jnh-tosi-4", "peak-mini", "peak-crown", "peak-fuji", "peak-rainier", "sun-home-equinox", "sun-home-eclipse-2", "sunray-sedona", "sunray-evansport", "sunray-aspen", "salus-laurel-king", "salus-element", "tylo-kiruna-2",
  ]);
});
...
test("the pilot scenario for two indoor infrared seats on 120 V returns ten known configurations", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "excluded").length, 229);
});
...
test("the public finder evaluates the complete pilot for six outdoor seats on 240 V", () => {
  ...
  assert.equal(results.length, 273);
...
test("adding a room envelope keeps the pilot honest when installation clearances are unknown", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "needs-verification").length, 44);
  ...
  assert.equal(roomVerification.length, 19);
});
