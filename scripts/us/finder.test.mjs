...
test("real pilot data produces eleven known indoor infrared 120 V results", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "needs-verification").length, 33);
  assert.equal(results.filter((entry) => entry.status === "excluded").length, 187);
...
test("the pilot scenario for two indoor infrared seats on 120 V returns seven known configurations", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "needs-verification").length, 28);
  assert.equal(results.filter((entry) => entry.status === "excluded").length, 196);
});
...
test("the public finder evaluates the complete pilot for six outdoor seats on 240 V", () => {
  ...
  assert.equal(results.length, 231);
...
test("adding a room envelope keeps the pilot honest when installation clearances are unknown", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "needs-verification").length, 35);
