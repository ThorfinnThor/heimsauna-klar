...
test("real pilot data produces eleven known indoor infrared 120 V results", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "needs-verification").length, 32);
  assert.equal(results.filter((entry) => entry.status === "excluded").length, 172);
...
test("the pilot scenario for two indoor infrared seats on 120 V returns seven known configurations", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "needs-verification").length, 27);
  assert.equal(results.filter((entry) => entry.status === "excluded").length, 181);
});
...
test("the public finder evaluates the complete pilot for six outdoor seats on 240 V", () => {
  ...
  assert.equal(results.length, 215);
