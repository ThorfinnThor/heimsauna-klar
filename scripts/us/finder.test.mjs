...
test("real pilot data produces fourteen known indoor infrared 120 V results", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "meets-known-criteria").length, 16);
  assert.equal(results.filter((entry) => entry.status === "needs-verification").length, 36);
  assert.equal(results.filter((entry) => entry.status === "excluded").length, 211);
  assert.deepEqual(results.filter((entry) => entry.status === "meets-known-criteria").map((entry) => entry.productId), [
    "peak-shasta", "peak-everest", "jnh-tosi-1", "jnh-tosi-2", "jnh-tosi-4", "peak-mini", "peak-crown", "peak-fuji", "peak-rainier", "sun-home-equinox", "sun-home-eclipse-2", "sunray-sedona", "sunray-evansport", "sunray-aspen", "salus-laurel-king", "salus-element",
  ]);
});
...
test("the pilot scenario for two indoor infrared seats on 120 V returns ten known configurations", () => {
  ...
  assert.deepEqual(
    ...
    ["peak-everest", "jnh-tosi-2", "jnh-tosi-4", "peak-crown", "peak-fuji", "sun-home-equinox", "sun-home-eclipse-2", "sunray-sedona", "sunray-evansport", "sunray-aspen", "salus-laurel-king", "salus-element"],
  );
  assert.equal(results.filter((entry) => entry.status === "needs-verification").length, 31);
  assert.equal(results.filter((entry) => entry.status === "excluded").length, 220);
});
...
test("the public finder evaluates the complete pilot for six outdoor seats on 240 V", () => {
  ...
  assert.equal(results.length, 263);
...
test("adding a room envelope keeps the pilot honest when installation clearances are unknown", () => {
  ...
  assert.equal(results.filter((entry) => entry.status === "needs-verification").length, 43);
  ...
  assert.equal(roomVerification.length, 18);
});
