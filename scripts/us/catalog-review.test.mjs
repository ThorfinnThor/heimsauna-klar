...
test("the current indexed first-wave catalog has a reproducible review report", () => {
  ...
  assert.equal(report.summary.products, 231);
  assert.equal(report.summary.candidatesAwaitingReview, 208);
  ...
  assert(report.blockers.includes("208 candidate or draft products still require their first technical review."));
