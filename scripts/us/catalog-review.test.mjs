...
test("the current indexed first-wave catalog has a reproducible review report", () => {
  ...
  assert.equal(report.summary.products, 215);
  assert.equal(report.summary.candidatesAwaitingReview, 192);
  ...
  assert(report.blockers.includes("192 candidate or draft products still require their first technical review."));
