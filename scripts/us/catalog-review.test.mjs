...
test("the current protected catalog has a reproducible review report", () => {
  ...
  assert.equal(report.summary.products, 18);
  assert.equal(report.summary.candidatesAwaitingReview, 18);
  ...
  assert(report.blockers.includes("18 candidate or draft products still require their first technical review."));
