...
test("the catalog index normalizes every real pilot product", () => {
  assert.equal(researchItems.length, 290);
...
test("catalog filters keep documented matches deterministic", () => {
  ...
  assert.equal(filterUsCatalogItems(researchItems, { ...defaultUsCatalogFilters, capacity: "4-plus" }).length, 115);
  assert.equal(filterUsCatalogItems(researchItems, { ...defaultUsCatalogFilters, query: "peak" }).length, 11);
  assert.equal(filterUsCatalogItems(researchItems, { ...defaultUsCatalogFilters, voltage: "120" }).length, 30);
});
