...
test("the catalog index normalizes every real pilot product", () => {
  assert.equal(researchItems.length, 115);
...
test("catalog filters keep documented matches deterministic", () => {
  assert.equal(filterUsCatalogItems(researchItems, { ...defaultUsCatalogFilters, placement: "outdoor" }).length, 80);
  assert.equal(filterUsCatalogItems(researchItems, { ...defaultUsCatalogFilters, capacity: "4-plus" }).length, 62);
