...
const waveIds = new Set(plan.second_wave?.product_ids ?? []);
if (waveIds.size !== 27) throw new Error(`Expected 27 second-wave products, found ${waveIds.size}`);
...
for (const configuration of configurations.configurations) {
  const entries = configurationsByProductId.get(configuration.product_id) ?? [];
  entries.push(configuration);
  configurationsByProductId.set(configuration.product_id, entries);
}
...
for (const productId of waveIds) {
  ...
  product.change_reason = "Sol-approved second US publication wave: exact source, technical data, product-specific copy, SEO and release checks completed.";
  for (const configuration of productConfigurations) configuration.publication_status = "published";
}

plan.second_wave.status = "published";
plan.second_wave.published_at = today;
plan.current_result.second_wave_indexable_now = waveIds.size;
plan.current_result.second_wave_editorial_reviewed = waveIds.size;
...
console.log(`Promoted ${waveIds.size} second-wave products; ${plan.current_result.public_products} US products are now public.`);
