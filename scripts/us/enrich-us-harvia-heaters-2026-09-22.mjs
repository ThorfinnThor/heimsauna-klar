...
for (const record of records) {
  ...
  sources.evidence.push(
    { id: pEvidence, entity_id: record.id, source_id: sourceId, field_path: "product_identity_heat_placement", raw_value: `${record.model} (${record.sku}) is listed by Harvia as an electric traditional sauna heater for the interior/wet-area use context.` },
    { id: cEvidence, entity_id: configuration.id, source_id: sourceId, field_path: "configuration_dimensions_materials", raw_value: `${record.model}: dimensions ${JSON.stringify(record.dimensions)}; ${record.weight ? `net weight ${JSON.stringify(record.weight)}; ` : "net weight is not stated; "}materials ${record.materials.join(", ")}; recommended room volume ${record.roomVolume}.` },
  ...
  product.placements = documented(record.placement, pEvidence);
  ...
  configuration.capacity = {
    seated: unknown("A heater is not a cabin and has no seated capacity."),
    reclining: unknown("A heater is not a cabin and has no reclining capacity."),
  };
  configuration.dimensions = {
    exterior: documented(record.dimensions, cEvidence),
    interior: unknown("Interior dimensions are not applicable to a standalone heater."),
  ...
  configuration.materials = documented(record.materials, cEvidence);
  ...
  configuration.electrical_supply_options = [{
    ...
    requirements: [{
      ...
      frequency_hz: documented(record.frequency, eEvidence),
...
if (!productIds.has(newRecord.id)) {
  ...
  products.products.push({
    ...
    product_type: documented("heater", pEvidence), heat_type: documented("hybrid", pEvidence), energy_sources: documented(["electric"], eEvidence), placements: documented(["indoor"], pEvidence), form: documented("Floor-standing electric combi sauna heater with evaporator", pEvidence), configuration_ids: [newRecord.configId], source_ids: [newRecord.sourceId], publication_status: "candidate", spec_checked_at: checkedAt, next_review_at: nextReviewAt, change_reason: "Added from the exact official Harvia US Concept R Combi product page; publication remains protected pending editorial and rights review.",
  });
